...
dnsx -l "$scan_pat/resolved.txt" -json -o "$scan_path/dns.json" | jq -r '.a?[]?' | anew "scan_path/ips.txt" | wc -l

#Port Scanning & HTTP Server Discovery 

nmap -T4 -vv -iL "$scan_path/ips.txt" --top-ports 3000 -n --open -oX "$scan_path/nmap.xml"
tew -x "$scan_path/nmap.xml" -dnsx "$sscan_path/dns.json" --vhost -o "$scan_path/hostport.txt" | httpx -sr -srd "$scan_path/responses" -json -o "$scan_path/http.json"
